			...
			<div className="flex sm:flex-row flex-col gap-10 justify-around w-full ">
				<div className="flex flex-col gap-8">
					...
					<div className="flex flex-col gap-4 sm:gap-8 ">
						<h2 className='sm:text-3xl text-2xl font-semibold'>Contact Info</h2>
						<p className="sm:text-xl text-lg text-amber-600 font-medium flex items-center gap-2 "><FaPhone /><span className='text-black cursor-pointer hover:text-white transition-all'>{AboutMeContents?.phone}</span></p>
						<p className="sm:text-xl text-lg text-amber-600 font-medium flex items-center gap-2 "><FaEnvelope /><span className='text-black cursor-pointer hover:text-white transition-all'>{AboutMeContents?.email}</span></p>
						<p className="sm:text-xl text-lg text-amber-600 font-medium flex items-center gap-2 "><FaHome /> <span className='text-black cursor-pointer hover:text-white transition-all'>{AboutMeContents?.place}</span></p>
					</div>
				</div>
				<div className="flex flex-col gap-8">
					<div className="flex flex-col gap-1">
						<h2 className='sm:text-3xl text-2xl font-semibold'>Quick Link's</h2>
						{NavLinks.map((link, i) => {
							return (
								<div className="flex  sm:text-2xl text-xl w-full p-2 gap-2  items-center" key={link.title}>
					...
					<div className="flex sm:gap-6 gap-2">
						{HomeContents.map((pro, i) => {
							return (
								<a href={pro?.link}>
									<button className="sm:text-3xl text-2xl rounded-full cursor-pointer hover:text-purple-700 " key={i}>
										{pro?.icon}
									</button>
								</a>
							)
